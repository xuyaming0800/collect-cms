define([ 'jquery', 'ejs','bootstrap','jqueryvalue' ], function($, ejs,bts,jv) {
	...
	var templateUrl="asset/template/";
	...
	var render={
			...
			renderDropItems : function(dropDataArray) {// 初始化左侧区域 必须同步 因为顺序问题 不采用AMD模式
				for(var i in dropDataArray){
					var _template =packageItemObject(
							renderDropItemHtml(templateUrl + dropDataArray[i].renderName + ".html",dropDataArray[i].renderJson),dropDataArray[i].renderName);
	...
	function renderDropItemHtml(template,renderJson){
		var _template = new EJS({
			url : template
		}).render(renderJson);
		var result={

package com.autonavi.cc.web.openapi.util;
...
import com.autonavi.cc.api.util.AES;
import com.autonavi.cc.api.util.PropertiesConfig;
			...
			try {
				...
				while (paramNames.hasMoreElements()) {
					...
					if (paramValues.length == 1) {
						...
						throw new BusinessException((int)ExceptionCode.TWO_PARAMS_WITH_SAME_NAME ,"不能有两个同名参数!");
					}
				}
				...
				try {
	                ...
	                throw new BusinessException((int)ExceptionCode.PARAM_VALUE_ERROR,"参数值不正确!" );
				}
							...
							if (!"encrypt".equals(key) && !"serviceid".equals(key) && !"encryptback".equals(key)){
								if(value instanceof String){
									...
									if((tempValue = AES.Decrypt(value.toString(), getDecryptKey()))==null){
										throw new BusinessException((int)ExceptionCode.DECRYPT_ERROR ,"解密错误!");
									}
									value = tempValue;
								}
							}
